<?php /*a:3:{s:68:"/www/wwwroot/lingzweb/public/../templates/default/cms/list_case.html";i:1628763224;s:65:"/www/wwwroot/lingzweb/public/../templates/default/cms/header.html";i:1628763224;s:65:"/www/wwwroot/lingzweb/public/../templates/default/cms/footer.html";i:1628763224;}*/ ?>
        ...
        <div class="content">
            ...
            <div class="product_box">
                <div class="classify-screen">
                    <?php $_result=filters($modelid,$catid);if(is_array($_result) || $_result instanceof \think\Collection || $_result instanceof \think\Paginator): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <div class="classify-box clearfix">
                        <h5 class="classify-name"><?php echo htmlentities($vo['title']); ?>：</h5>
                        <div class="classify-screen-con">
                            <div class="classify-choose">
                                <?php if(is_array($vo['options']) || $vo['options'] instanceof \think\Collection || $vo['options'] instanceof \think\Paginator): $i = 0; $__LIST__ = $vo['options'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vs): $mod = ($i % 2 );++$i;?>
                                <a target="_self" href="<?php echo htmlentities($vs['url']); ?>" <?php if($vs['active']): ?>class="selected"<?php endif; ?>><span><?php echo htmlentities($vs['title']); ?></span></a>
                                <?php endforeach; endif; else: echo "" ;endif; ?>
                            </div>
                        </div>
                    </div>
            ...
            <!--E 筛选列表 -->
            <!--S 筛选条件 -->
            <?php $where=structure_filters_sql($modelid); ?>
            <!--E 筛选添加 -->
            <div class="case_ny_list">
            	<!--S 文章列表-->
                <ul class="clearfix">
                    <?php $cache = 3600;$cacheID = to_guid_string(array('module'=>'cms','action'=>'lists','where'=>$where,'catid'=>$catid,'cache'=>'3600','order'=>'listorder DESC','num'=>'8','page'=>$page,'return'=>'data',));if($cache && $_return = Cache::get($cacheID)):$data = $_return;else: $cmsTagLib =  \think\Container::get("\\app\\cms\\taglib\\CmsTagLib");if(method_exists($cmsTagLib, "lists")):$data = $cmsTagLib->lists(array('module'=>'cms','action'=>'lists','where'=>$where,'catid'=>$catid,'cache'=>'3600','order'=>'listorder DESC','num'=>'8','page'=>$page,'return'=>'data',));if($cache):Cache::set($cacheID, $data, $cache);endif;endif;endif;$pages = $data->render(); if(is_array($data) || $data instanceof \think\Collection || $data instanceof \think\Paginator): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 4 );++$i;?>
                    <li <?php if($mod == '3'): ?>style="margin-right:0;" <?php endif; ?>> <div class="_con">
                        <div class="_img"><img src="<?php echo htmlentities((isset($vo['thumb']) && ($vo['thumb'] !== '')?$vo['thumb']:'https://via.placeholder.com/270x210')); ?>" alt="<?php echo htmlentities($vo['title']); ?>"></div>
                            <div class="_info">
                                <h3><?php echo htmlentities(str_cut($vo['title'],12)); ?></h3>
                                <div class="txt">
                                    <?php echo htmlentities(str_cut($vo['description'],50)); ?>
                                </div>
                                <a href="<?php echo htmlentities($vo['url']); ?>" class="_more">查看更多<i>&gt;</i></a>
                            </div>
                        </div>
                    </li>
                    <?php endforeach; endif; else: echo "" ;endif; ?>
